...
void tests()
{
    ...
    Maths::BiInterpolate(mat, 0.5, 0.5, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 1);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 0, 0, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 2);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 1, 0, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 3);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 0, 1, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 4);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 1, 1, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 1.5);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 0.5, 0, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 2.8);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 0, 0.9, res);
    EXPECT_TRUE(array_match(exp, res, channels));

    std::fill(exp, exp+channels, 2.65);
    std::fill(res, res+channels, 0);
    Maths::BiInterpolate(mat, 0.33, 0.66, res);
    EXPECT_TRUE(array_match(exp, res, channels));
}

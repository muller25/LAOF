...
// TODO: not thread safe; separate begin/end so that end doesn't have to check for query frequency
extern "C" inline gcoclock_t gcoclock() 
{
	gcoclock_t result = 0;
...
OLGA_INLINE void GCoptimization::addterm2_checked(EnergyT* e, VarID i, VarID j, EnergyTermType e00, EnergyTermType e01, EnergyTermType e10, EnergyTermType e11, EnergyTermType w)
{
	...
	if ( e00+e11 > e01+e10 )
    {
        printf("e00 %.6f, e01 %.6f e10 %.6f e11 %.6f w %.6f\n", e00, e01, e10, e11, w);
	...
	if ( m_numNeighbors && m_needTodeleteNeighbors )
	{
		for ( SiteID i = 0; i < m_num_sites; i++ )
		{
			if (m_numNeighbors[i] != 0 )
			{
                ...
                m_neighborsIndexes[i] = NULL;
                m_neighborsWeights[i] = NULL;
			}
		}
        ...
        m_numNeighbors = NULL;
        m_neighborsIndexes = NULL;
        m_neighborsWeights = NULL;
	}
}
...
void GCoptimizationGeneralGraph::finalizeNeighbors()
{
    ...
    assert(tempWeights != NULL && tempIndexes != NULL);
    ...
    assert(m_numNeighbors != NULL && m_neighborsWeights != NULL && m_neighborsIndexes != NULL);
	...
	for ( site = 0; site < m_num_sites; site++ )
	{
		if ( m_neighbors && !m_neighbors[site].isEmpty() )
		{
            ...
            assert(m_neighborsWeights[site] != NULL && m_neighborsIndexes[site] != NULL);
